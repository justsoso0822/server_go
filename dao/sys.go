...
func GetAllMemConfig(ctx context.Context) ([]model.MemConfig, error) {
	rows, err := q(ctx).MemConfig.WithContext(ctx).Find()
	return derefSlice(rows), err
}

func GetMemConfigValue(ctx context.Context, id int) (string, error) {
	c := q(ctx).MemConfig
	row, err := c.WithContext(ctx).Where(c.ID.Eq(int32(id))).First()
...
func IsGm(ctx context.Context, uid int64) (bool, error) {
	g := q(ctx).SysGm
	count, err := g.WithContext(ctx).Where(g.UID.Eq(int32(uid))).Count()
	return count > 0, err
}
